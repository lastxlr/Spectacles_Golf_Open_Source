...
function resumeTween(tweenObject, _tweenName) {
    ...
    if (tweenScriptComponent) {
        ...
        if (tweenScriptComponent.playAll && tweenScriptComponent.tweenType == "chain") {
            ...
            debugPrint("Tween Manager: Warning, trying to resume " + tweenName + " on " + tweenObject.name + " object, which hasn't been initialized", true);
        }
    } else {
        debugPrint("Tween Manager: Trying to resume " + tweenName + " on " + tweenObject.name + " object, which does not exist. Ensure that the Tween Type has been initialized and started.", true);
    }
}
...
function pauseTween(tweenObject, _tweenName) {
    ...
    if (tweenScriptComponent) {
        ...
        if (tweenScriptComponent.tweenType == "chain" && tweenScriptComponent.playAll) {
            ...
            debugPrint("Tween Manager: Warning, trying to pause " + tweenName + " on " + tweenObject.name + " object, which hasn't been initialized", true);
        }
    } else {
        debugPrint("Tween Manager: Trying to pause " + tweenName + " on " + tweenObject.name + " object, which does not exist. Ensure that the Tween Type has been initialized and started.", true);
    }
}
...
function isPaused(tweenObject, tweenName) {
    ...
    debugPrint("TweenManager: You are trying to check if " + tweenName + " is currently paused, but a Tween of that type does not exist on " + tweenObject.name + " object.", true);
}
...
function isPlaying(tweenObject, tweenName) {
    ...
    debugPrint("TweenManager: You are trying to check if " + tweenName + " is currently playing, but a Tween of that type does not exist on " + tweenObject.name + " object.", true);
}
...
function startTween(tweenObject, tweenName, completeCallback, startCallback, stopCallback) {
    var tweenScriptComponent = findTween(tweenObject, tweenName);
    if (tweenScriptComponent) {
        debugPrint("Tween Manager: Starting " + tweenName  + " on " + tweenObject.name + " object.");
        ...
        // Start the tween
        if (!tweenScriptComponent.startTween) {
            debugPrint("Tween Manager: Warning, trying to start " + tweenScriptComponent.tweenName + " on " + tweenObject.name + " object, which hasn't been initialized", true);
            return;              
        }
...
function stopTween(tweenObject, tweenName) {
    var tweenScriptComponent = findTween(tweenObject, tweenName);
    if (tweenScriptComponent) {
        debugPrint("Tween Manager: Stopping " + tweenName + " on " + tweenObject.name + " object.");
        if (tweenScriptComponent.tweenType == "chain") {
            if (tweenScriptComponent.playAll && tweenScriptComponent.allTweens) {
                ...
                debugPrint("Tween Manager: Warning, trying to stop " + tweenName + " on " + tweenObject.name + " object, which hasn't been started");
            }
...
function setStartValue(tweenObject, tweenName, startValue) {
    ...
    if (tweenScriptComponent) {
        if (tweenScriptComponent.setStart) {
            ...
            debugPrint("Tween Manager: You cannot manually set the start value of " + tweenName + " on " + tweenObject.name + " object.");
        }
    }
}
...
function setEndValue(tweenObject, tweenName, endValue) {
    ...
    if (tweenScriptComponent) {
        if (tweenScriptComponent.setEnd) {
            ...
            debugPrint("Tween Manager: You cannot manually set the end value of " + tweenName + " on " + tweenObject.name + " object.");
        }
    }
}
...
function resetObject(tweenObject, tweenName) {
    var tweenScriptComponent = findTween(tweenObject, tweenName);
    if (tweenScriptComponent) {
        debugPrint("Tween Manager: Resetting Object " + tweenName + " on " + tweenObject.name + " object.");
...
function resetTweenComponent(tweenScriptComponent) {
    if (tweenScriptComponent) {
        debugPrint("Tween Manager: Resetting tween " + tweenScriptComponent.tweenName + " on " + tweenScriptComponent.getSceneObject().name + " object.");

        if (tweenScriptComponent.tweenType == "chain") {
            tweenScriptComponent.backwards = false;
            debugPrint("Tween Manager: Chain Tween reset is not fully supported" + " on " + tweenScriptComponent.getSceneObject().name + " object.");
        }

        if (tweenScriptComponent.movementType && tweenScriptComponent.movementType > 0) {
            debugPrint("Tween Manager: Reset for this tween movement type is not fully supported" + " on " + tweenScriptComponent.getSceneObject().name + " object.");
        }
...
function restartAutoTweens() {
    for (var i = 0; i < script.registry.length; i++) {
        ...
        if (tweenScriptComponent && tweenScriptComponent.playAutomatically) {
            debugPrint("Restarting tween " + tweenScriptComponent.tweenName + " on " + tweenScriptComponent.getSceneObject().name + " object.");

            // Start the tween
            if (!tweenScriptComponent.startTween) {
                debugPrint("Tween Manager: Warning, trying to start " + tweenScriptComponent.tweenName + " on " + tweenScriptComponent.getSceneObject().name + " object, which hasn't been initialized", true);
                return;              
            }
            tweenScriptComponent.startTween();
        }
...
function addToRegistry(tweenScriptComponent) {
    if (tweenScriptComponent) {
        debugPrint("Adding tween " + tweenScriptComponent.tweenName + " on " + tweenScriptComponent.getSceneObject().name + " object to Tween Manager registry");
...
function findTween(tweenObject, tweenName) {
    ...
    for (var i = 0; i < scriptComponents.length; i++) {
        ...
        if (scriptComponent) {
            ...
            debugPrint("Tween Manager: Tween type hasn't initialized. Ensure that " + tweenName + " on " + tweenObject.name + " object is on \"Lens Turn On\" and that Tween Manager is at the top of the Objects Panel.", true);
            return;
        }
    }

    debugPrint("Tween Manager: Tween, " + tweenName + ", is not found. Ensure that " + tweenName + " on " + tweenObject.name + " object is on \"Lens Turn On\" and that Tween Manager is at the top of the Objects Panel.", true);
}
...
function getGenericTweenValue(tweenObject, tweenName) {
    ...
    for (var i = 0; i < scriptComponents.length; i++) {
        ...
        if (scriptComponent) {
                ...
                if (scriptComponent.tweenType == "value") {
                    if (scriptComponent.tween) {
                        if (!scriptComponent.tween._isPlaying) {
                            debugPrint("Tween Manager: Tween Value, " + tweenName + " on " + tweenObject.name + " object, is not currently playing. Ensure that it has been started by either calling its startTween() function or by setting it to Play Automatically.", true);
                        }
                    } else {
                        debugPrint("Tween Manager: Tween Value, " + tweenName + " on " + tweenObject.name + " object, has not been set up. Ensure that this Tween Value is ordered before every other script that uses it in the Objects Panel and Inspector. Try initializing it in the Initialized event and scripting in the Lens Turn On event.", true);
                    }
            ...
            debugPrint("Tween Manager: Tween Value, " + tweenName + " on " + tweenObject.name + " object, hasn't initialized. Needs to initialize prior to scripting playback. Likely an order of operations issue. Try initializing tween type in the Initialized event and scripting it in the Lens Turn On event. Or, try moving the Tween Manager to the top of the Objects hierarchy.", true);
            return;
        }
    }

    debugPrint("Tween Manager: Tween Value, " + tweenName + " on " + tweenObject.name + " object, is not found. Ensure that " + tweenName + " is enabled and the Tween Name passed into this function exactly matches the specified Tween Name for this Tween Value in the Inspector.", true);
}
